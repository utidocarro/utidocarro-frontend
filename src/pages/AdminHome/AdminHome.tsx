...
const AdminHomePage: React.FC = () => {
  ...
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string | null>(null);
  // Novo estado para armazenar alterações pendentes (comentário e status)
  const [pendingChanges, setPendingChanges] = useState<{ [key: number]: { comentario?: string; status?: OrdemServico['status'] } }>({});
  ...
  // Função para lidar com a mudança no comentário
  ...
  const handleSave = async (osId: number) => {
    const changes = pendingChanges[osId];
    if (!changes) {
    ...
    try {
      ...
      await api.patch(`/api/ordemServico/${osId}`, changes);
      
      // Remove as alterações pendentes para esta OS após o salvamento bem-sucedido
      setPendingChanges(prev => {
      ...
      console.error(`Erro ao salvar alterações para OS ${osId}:`, error);
        ...
        <table>
          <thead>
            <tr>
              ...
              <th>Data</th>
              <th>Comentário</th>
              <th>Ações</th> {/* Nova coluna para o botão Salvar */}
            </tr>
          </thead>
          <tbody>
            {ordensFiltradas.length > 0 ? (
                ...
                <tr key={os.id}>
                  ...
                  <td>
                    ...
                    <button
                      ...
                      // Desabilita o botão se não houver alterações pendentes para esta OS
                      disabled={!pendingChanges[os.id] || Object.keys(pendingChanges[os.id]).length === 0}
                    >
                      Salvar
                    </button>
                  </td>
